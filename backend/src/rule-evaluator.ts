...
// Agent status names for logging
...
export class RuleEvaluator {
  ...
  async evaluateAgent(agentId: bigint): Promise<EvaluationResult[]> {
    ...
    try {
      ...
      logger.info({
        ...
        status: AgentStatusNames[agent.status] || agent.status,
        depositedAmount: formatEther(agent.depositedAmount),
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        logger.info({
          ...
          lastExecuted: rule.lastExecuted === 0n ? 'Never' : new Date(Number(rule.lastExecuted) * 1000).toISOString(),
        }, 'Checking rule');

        if (!rule.enabled) {
          logger.debug({ agentId: agentId.toString(), ruleIndex: i }, 'Rule disabled, skipping');
          continue;
        }
  ...
  private evaluateTimeWeighted(
    ...
    logger.info({
      agentId: baseResult.agentId.toString(),
      ruleIndex: baseResult.ruleIndex,
      ruleType: 'TIME_WEIGHTED',
      now: new Date(Number(now) * 1000).toISOString(),
      lastExecuted: rule.lastExecuted === 0n ? 'Never' : new Date(Number(rule.lastExecuted) * 1000).toISOString(),
      timeSinceLastExecution: `${timeSinceLastExecution}s`,
      cooldown: `${rule.cooldown}s`,
      shouldExecute,
    }, 'TIME_WEIGHTED evaluation details');
